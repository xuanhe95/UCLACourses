...
string Player::move(int dir)
{
    ...
    if (hasRabbit(row(), col())) return "Player walked into a rabbit and died.";
...
void Arena::moveRabbits()
{
    ...
    for (int i = 0; i < m_nRabbits; i++) {
        Rabbit* curRabbitPtr = m_rabbits[i];
        curRabbitPtr->move();
        if (curRabbitPtr->row() == m_player->row() && curRabbitPtr->col() == m_player->col()) m_player->setDead();  // player dead
