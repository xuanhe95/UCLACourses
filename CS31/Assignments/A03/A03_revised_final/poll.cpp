...
int countSection(string pollSection, char party, int& seatCount) {
	...
	while (endIndex > 0) {
		...
		int cnt = myAtoi(data.substr(startIndex, endIndex - startIndex));	//get the seats of party
		seat += cnt;
		seatCount += cnt;

		data = data.substr(endIndex + 1, data.size() - endIndex);
		endIndex = data.find(party);	//find party code index
	}
	return seat;
}
