...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    for (int unsorted = 1; unsorted < studentsPtr.size(); unsorted++)
    {
        Student* nextItem = studentsPtr[unsorted];
        int loc = unsorted;
        while (loc > 0  &&  comp(nextItem, studentsPtr[loc-1]))
        {
            studentsPtr[loc] = studentsPtr[loc-1];
            loc--;
        }
        studentsPtr[loc] = nextItem;
    }
